...
class AuthController extends Controller
{
    ...
    public function install(Request $request)
    {
        ...
        try {
            ...
            if ($request->has('external_install')) {
                ...
                return new Response($errorMsg, $e->getResponse()->getStatusCode());
            }
        }
    }
    ...
    public function handleConfiguration(Request $request)
    {
        if ($request->has('store_hash') && $request->has('access_token') && $request->has('context') && $request->has('shopkey')) {
            ...
            $xmlResponse = $this->makeBigCommerceAPIRequest("GET", "stores/" . $this->getStoreHash() . "/v2/store");
            $collection = $this->mapXmlResponseToCollection($xmlResponse);
            ...
            if ($collection["features"]["stencil_enabled"]) {
                ...
                if ($scriptRow["id"]) {
                    $uuid = $scriptRow["uuid"];
                    $deleteResponse = $this->makeBigCommerceAPIRequest("DELETE", "stores/" . $this->getStoreHash() . "/v3/content/scripts/$uuid");
                    $scriptRow->delete();
                } else {
                    ...
                    if (isset($active_status)) {
                        ...
                        // making request
                        $response = $this->makeBigCommerceAPIRequest("POST", "stores/" . $this->getStoreHash() . "/v3/content/scripts", $findologic_script);
                        // converting data
    ...
    private function makeBigCommerceAPIRequest($method, $endpoint, $body = '')
    {
        $requestConfig = [
            'headers' => [
        ...
        try {
            $result = $this->httpClient->request($method, 'https://api.bigcommerce.com/' . $endpoint, $requestConfig);
            return $result;
        } catch (RequestException $e) {
    ...
    private function mapXmlResponseToCollection($XMLResponse)
    {
